...
{   
    ...
    {
        if(this.readyState == 4 && this.status == 200 ) // State = 4 means Operation to GET the file is completed and 200 means HTTP success
        {
            allText = this.responseText; // Command for retrieving file contents
            lines = allText.split(/[\s]+/); // Parsing a CSV file
            for (i=0; i<lines.length; i++) // Looping through each line in CSV file
            {       
                b=lines[i].split(","); // Splitting a line and creating an array of elements seperated  by commas
                if (b[0]==btn) // check for a match of "Root Word"
                {      
                    var numWords=b.length; // Retrieve total number of Derived words
                    for (j=1; j<numWords; j++) // Create a list of Derived words
                    {   
...
{       
    ...
    req=req.concat(word,"?key=<YOUR KEY HERE>");
    ...
    {       
        ...
        {               
            ...
            if(xmlDoc.getElementsByTagName('sens').length >0) // Search for element "sens" in the XML, as it contains the meaning
            {
            ...
            if(xmlDoc.getElementsByTagName('syn').length >0) // Search for element "syn" in the XML, as it contains synonyms
            {
                ...
                for (j=0; j<numWords; j++) // Create a list of Synonyms
                {   
                    val = b[j].trim();
                    valfinal = val.split(" ")[0];
                    if (valfinal !== word) // Avoid displaying the synonym if it matches the user entered word
                    {
                        var node = document.createElement("li");
...
{       
    ...
    {       
        ...
        {               
            ...
            if(xmlDoc.getElementsByTagName('sens').length >0 && xmlDoc.getElementsByTagName('mc').length >0 ) 
            {
                ...
                */
                if(xmlDoc.getElementsByTagName('vi').length >0)
                {
                    ...
                    for(var i = 0; i < datsent.childNodes.length; i++)
                    {
                        ...
                        */
                        if (datsent.getElementsByTagName('it').length > 0 && datsent.childNodes[i].nodeValue == null) 
                        {
...
function flipword()
{
    ...
    // Start a new code block by inserting new braces <Reason Unknown>
    {
        ...
        // Using XMLHttp to call an API for obtaining information
        req="https://www.dictionaryapi.com/api/v1/references/thesaurus/xml/";
        req=req.concat(word_front,"?key=<YOUR KEY HERE>"); 
        var wsreq=new XMLHttpRequest();
        // Open and Send the API request
        wsreq.open("GET",req,true);
        wsreq.send();        
...
{               
    ...
    rawFile.onreadystatechange = function ()
    {
        if(rawFile.readyState === 4)
        {
            if(rawFile.status === 200 || rawFile.status == 0)
            {
                ...
                for (i=0; i<rows.length; i++) // Loop through all the lines
                {       
                    b=rows[i].split(","); // Split each line by using comma as a seperator
                    if (b[0]=="SET") // Hardcoded the "SET" value for now TODO: Improve it
                    {       
    ...
    wordAud = document.getElementById("wordfront").innerHTML; // retrieving the word from the flashcard front for generating audio file
    req = "https://www.dictionaryapi.com/api/v1/references/collegiate/xml/";
    req = req.concat(wordAud,"?key=<YOUR KEY HERE>"); 
    var wsreq = new XMLHttpRequest();
    // Open and Send the API request
    wsreq.open("GET",req,true);
    wsreq.send();        
    ...
    {       
        ...
        {               
            ...
            if(xmlDoc.getElementsByTagName('sound').length >0) // Search for element "sens" in the XML, as it contains the meaning
            {
                var data = xmlDoc.getElementsByTagName('wav')[0]; // File name can be accessed from this node
                var soun = data.childNodes[0];
                audiofc = soun.nodeValue; // WAV file name containg the audio of the flash card word
                firstAlpha = audiofc.charAt(0); // Accessing the first character from the file name, in the process of creating the URL
                audioURL = "https://media.merriam-webster.com/soundc11/" + firstAlpha + "/" + audiofc; // Creating URL for audio access
                audio = document.getElementById("audio"); // Accessing audio element for loading purpose
                var audioSrc = document.getElementById("wordAudio"); // Accessing source element for dynamically assigning "src" value 
                audioSrc.setAttribute("src",audioURL); // Assing "src" value for accessing the audio file
                audio.load(); // Loading the audio file to be played by the user
...
function Wotd()
{
    ...
    // Using XMLHttp to call an API for obtaining information
    req="https://www.dictionaryapi.com/api/v1/references/thesaurus/xml/";
    req=req.concat(WOTD,"?key=<YOUR KEY HERE>"); 
    var wsreq=new XMLHttpRequest();
    // Open and Send the API request
    wsreq.open("GET",req,true);
    wsreq.send();        
    ...
    {       
        ...
        {               
            ...
            {
                var node = document.createElement("li");
                var text = document.createTextNode("Take a Break! Eat a Cake! No Words to Take!");
                node.appendChild(text);
